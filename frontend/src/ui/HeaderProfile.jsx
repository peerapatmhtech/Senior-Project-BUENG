...
const HeaderProfile = ({ className = "", isFriend }) => {
  const { logout } = useAuth();
  const queryClient = useQueryClient();
  const userEmail = localStorage.getItem("userEmail");

  // 1. Fetch user profile with useQuery, replacing localStorage
  const { data: userProfile } = useQuery({
    ...
    onSuccess: (data) => {
      // Keep localStorage in sync if needed by other parts of the app
      if (data) {
